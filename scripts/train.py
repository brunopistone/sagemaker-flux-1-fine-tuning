...
def load_text_encoders(class_one, class_two):
    text_encoder_one = class_one.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="text_encoder", revision=args.revision, variant=args.variant, force_download=True, cache_dir="/tmp/.cache"
    )
    text_encoder_two = class_two.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="text_encoder_2", revision=args.revision, variant=args.variant, force_download=True, cache_dir="/tmp/.cache"
    )
...
def main(args):
    ...
    noise_scheduler = FlowMatchEulerDiscreteScheduler.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="scheduler", use_cache=False, cache_dir="/tmp/.cache"
    )
    ...
    transformer = FluxTransformer2DModel.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="transformer", revision=args.revision, variant=args.variant, use_cache=False, cache_dir="/tmp/.cache"
    )
    ...
    if accelerator.is_main_process:
        ...
        FluxPipeline.save_lora_weights(
            save_directory="/opt/ml/model",
            transformer_lora_layers=transformer_lora_layers,
            text_encoder_lora_layers=text_encoder_lora_layers,
        )

        # Test LoRA adapter merge
        pipeline = FluxPipeline.from_pretrained(
            args.pretrained_model_name_or_path,
            revision=args.revision,
            variant=args.variant,
        ...
        pipeline.load_lora_weights("/opt/ml/model")
